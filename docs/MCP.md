...
Heritage AI Services exposes a Model Context Protocol (MCP) server at `/mcp`. Employees connect Claude Desktop — or any MCP-compatible client — using a personal token. Claude then has access to the compiled wiki, raw source documents, and AI skills, all filtered to the employee's permission scope.
...
Add the Heritage AI Services server:

```json
{
  "mcpServers": {
    "heritage-ai": {
      "url": "https://your-heritage-ai-server/mcp",
      "headers": {
...
Heritage AI Services tools will appear in Claude's tool list. The employee's wiki knowledge is immediately available.
...
## Using Heritage AI Services with Claude.ai (remote MCP)

Claude.ai supports remote MCP servers. Add Heritage AI Services as a remote server with the same URL and token:
...
| Connection refused | Ensure the Heritage AI Services API is running and accessible from the client network |
| HTTPS certificate errors | Configure a valid TLS certificate on your Heritage AI Services server |
