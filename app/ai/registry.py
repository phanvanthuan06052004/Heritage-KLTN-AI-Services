...
def _get_embedding_class(provider: ProviderType) -> type[EmbeddingProvider]:
    ...
    elif provider in (ProviderType.OPENAI, ProviderType.NINEROUTER):
...
class ProviderRegistry:
    ...
    async def _load_embedding_config(
        ...
        if spec.provider == ProviderType.NINEROUTER.value:
            base_url = base_url or await svc.get("ninerouter_url")
    ...
    async def _load_config(self, capability: str) -> ProviderConfig:
        ...
        if provider_str == ProviderType.NINEROUTER.value:
            base_url = base_url or await svc.get("ninerouter_url")
...
SUPPORTED_PROVIDERS = {
    ...
    "llm": [
        ...
        {"id": "ninerouter", "name": "9Router", "models": [
            "ModelFree-high", "model-premium", "Gemini", "Groq",
        ]},
