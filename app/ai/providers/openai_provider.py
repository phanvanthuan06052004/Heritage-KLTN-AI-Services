...
def _openai_compatible_base_url(config: ProviderConfig) -> Optional[str]:
    """9Router stores its root URL, while OpenAI SDK expects the /v1 base."""
    base_url = config.base_url
    if config.provider == ProviderType.NINEROUTER and base_url:
    ...
    def client(self):
        if self._client is None:
            ...
            self._client = openai.AsyncOpenAI(
                ...
                base_url=_openai_compatible_base_url(self.config),  # None = default OpenAI
    ...
    async def test_connection(self) -> tuple[bool, str]:
        try:
            result = await self.generate(
                ...
                system="Answer directly. Do not include reasoning.",
                max_tokens=128,
                temperature=0,
            )
