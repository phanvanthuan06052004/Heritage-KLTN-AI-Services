...
The Heritage AI Services wiki is the primary knowledge surface. Instead of storing raw document chunks, Heritage AI Services compiles documents into structured, interlinked wiki pages — written by an LLM agent, enriched by every new document you add.
...
Wiki pages are linked via `[[wikilinks]]` in their content. Heritage AI Services extracts these links into a `wiki_links` table, enabling:
