"""
Heritage AI Services — Document Import & Semantic Search (powered by Heritage AI Services).
...
async def lifespan(app: FastAPI):
    ...
    async with mcp_http_app.lifespan(app):
        logger.info("Starting Heritage AI Services (powered by Heritage AI Services)...")
        ...
        if settings.secret_key == "change-me-to-a-random-secret-string":
            logger.warning("SECRET_KEY is set to the default value - change it before deploying to production!")
        if settings.default_admin_password == "admin123":
            logger.warning("DEFAULT_ADMIN_PASSWORD is 'admin123' - change the admin password after first login!")

        # MCP server ready
        logger.success("Heritage AI Services MCP Server ready at /mcp")
        logger.success("Heritage Bridge endpoints ready at /api/heritage/*")
        logger.success("Heritage AI Services API started successfully")
        yield

        logger.info("Heritage AI Services shutdown complete")


app = FastAPI(
    title="Heritage AI Services",
    description=(
        "Heritage Cultural Knowledge Base - Document Import & Semantic Search Service "
        "(powered by Heritage AI Services). Bridges Heritage-LastDance-BE (NestJS) with AI knowledge pipeline."
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
# Heritage-specific bridge router (simplifies integration with Heritage-LastDance-BE)
from app.routers import heritage_bridge  # noqa: E402
...
# Heritage bridge: /api/heritage/import, /api/heritage/search, /api/heritage/wiki
app.include_router(heritage_bridge.router, prefix="/api", tags=["heritage-bridge"])


@app.get("/")
async def root():
    return {
        "name": "Heritage AI Services",
        "description": "Heritage Cultural Knowledge Base - Document Import & Semantic Search",
        "version": "1.0.0",
        "mcp_endpoint": "/mcp",
        "heritage_bridge": {
            "import": "/api/heritage/import",
            "search": "/api/heritage/search",
            "wiki_list": "/api/heritage/wiki",
            "wiki_detail": "/api/heritage/wiki/{slug}",
        },
        "docs": "/docs",
    }
